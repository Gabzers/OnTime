...
- **Validation checklist whenever a UI string is added/changed**: add the key to both `PtPT()` and `EnUS()` in `I18nController.cs`; mirror it in `OnTimeCRM_Frontend/src/utils/i18nFallback.ts` (pt-PT only — there's no en-US fallback file, English always depends on the live API call succeeding); use `t['YOUR.KEY']` in the component, never a literal. Quick sanity check before ending a session: `curl 'http://localhost:8080/api/i18n?locale=pt-PT' | jq '.map | length'` vs `...locale=en-US...` — they should match; a mismatch means a key exists in one locale's dictionary but not the other.
