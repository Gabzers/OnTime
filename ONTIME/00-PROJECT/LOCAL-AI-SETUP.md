...
Hub: [[OnTime]] · Maintenance & context rules: [[HOW-TO-USE]].
...
- [ ] Node 20+ (`node -v`).
- [ ] VS Code (or Rider).
- [ ] Repo cloned; open the **monorepo root as the workspace** so code + `ONTIME/` vault are both visible.

**Baseline (must all pass before installing):**
```bash
docker-compose up -d
cd OnTime_Backend && dotnet build && dotnet test
cd ../OnTime_Frontend && npm install && npm run build
```
...
run the model in Ollama, open the whole `OnTime/` folder (code **and** this vault) in the
...
Open the repo root (`OnTime/`) as the workspace so both the .NET/React code AND `ONTIME/` (this vault) are on disk for the extension. Then, per request, give context:
...
```
@file ONTIME/00-PROJECT/CONVENTIONS.md
@file ONTIME/00-PROJECT/DOMAIN.md          (or the relevant SCHEMA-*/feature note)
...
## Related
[[HOW-TO-USE]] · [[CONVENTIONS]] · [[ROADMAP]] · [[OnTime]]
