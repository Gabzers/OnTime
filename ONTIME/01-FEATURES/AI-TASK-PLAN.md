...
Hub: [[OnTime]] · Loop & gate: [[AI-WORKFLOW]] · Backlog: [[KNOWN-ISSUES]].
...
- **Do:** add Vitest + React Testing Library + jsdom to `OnTime_Frontend`; `test` script; one trivial sample test.
...
- **T14** ✅ 2026-06-06 — Require ≥1 vehicle at proposal creation (backend). New `PROPOSAL_MISSING_VEHICLE` (422) error in `ApiErrorCatalog`; validated in `ClientService.CreateAsync` (inline proposal) and `ProposalService.CreateForClientAsync` (standalone). Existing trade-in-only test updated to include a placeholder vehicle since a proposal can no longer exist with zero vehicles. 4 new backend tests green (118 total). Convert-to-sale can no longer 422 on `SALE_MISSING_VEHICLE` after the fact — the check now happens at creation time.
- **T15** ✅ 2026-06-06 — Vehicles screen status dot + active toggle. `VehicleModel.IsActive` already existed (inherited from `BaseEntity`, defaults `true`) — vault note was correct, just not visible by grepping the entity file directly. Added: `IsConfigured` computed in `VehicleRepository.GetModelsAsync` (`Versions.Any(v => ExternalColors not null/empty/"[]")`); `SetVehicleModelActiveRequest` DTO; `PATCH /api/vehicles/models/{id}/active` (ManagerOnly, mirrors `BrandsController.SetActive`); `IVehicleService.SetModelActiveAsync`. Frontend: `getVehicleStatusColor()` pure helper (grey=!configured, red=!active, green=both) + dot rendered in table/mobile card; `Switch` toggle (managers only) calling `vehiclesApi.setModelActive`. 4 new backend tests (122 total), 3 new frontend tests (15 total).

## Phase 1+ (same loop, larger units)
- **F1** ✅ 2026-06-06 — User Brands filter ([[USER-BRANDS]]). Decisions confirmed: server-side filtering, empty selection = all brands, each user self-manages. Implemented `UserVehicleBrand` (own-Id join entity, unique index on `user_id+vehicle_brand_id`, cascade delete) + `GET/PUT /api/users/me/vehicle-brands` + `VehicleRepository.GetModelsAsync(p, userId, ct)` defaults to the caller's selected brands when no explicit `brandId` is given (explicit `brandId` always wins). Frontend: "Marcas que Vendo" multi-select card on `ProfilePage`. 6 new backend tests (128 total); no new frontend tests needed (no new pure logic, just wiring — covered by TS + build). Known gap: the brand dropdown inside the proposal vehicle-picker (`VehicleProposalTable`) isn't filtered to the user's brands — low priority, documented in [[USER-BRANDS]].
...
## Related
[[AI-WORKFLOW]] · [[ROADMAP]] · [[KNOWN-ISSUES]] · [[TESTING]] · [[2026-05-30-access-scope-helper]] · [[OnTime]]
