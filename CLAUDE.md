# OnTime — Claude Code Context

Car-dealership CRM SaaS (Portugal). Monorepo:
- `OnTime_Backend/` — ASP.NET Core 8 + EF Core + PostgreSQL
- `OnTime_Frontend/` — React 18 + TypeScript + Vite + Ant Design 5
- `ONTIME/` — Obsidian vault: **the source of truth** (read it)

## The vault is the source of truth — read it, keep it updated

Facts (enums, schema, endpoints, business rules) live in `ONTIME/`, NOT in this file.
Do not duplicate them here — they drift.

**At the start of a task:** read the relevant notes.
- Entry point / index: `ONTIME/OnTime.md`
- Plan & priorities: `ONTIME/01-FEATURES/ROADMAP.md`
- What's done/broken: `ONTIME/00-PROJECT/STATUS.md`
- Entities + canonical enums: `ONTIME/00-PROJECT/DOMAIN.md`
- Patterns + file-size limits: `ONTIME/00-PROJECT/CONVENTIONS.md`
- Layers/auth/middleware: `ONTIME/00-PROJECT/ARCHITECTURE.md`
- Endpoints: `ONTIME/02-DATABASE/API-REFERENCE.md`
- Schema: `ONTIME/02-DATABASE/SCHEMA-*.md`
- Known frontend bugs: `ONTIME/02-DATABASE/FRONTEND-ISSUES.md`

**When anything changes — before ending the session — update the matching note** (table in `ONTIME/00-PROJECT/HOW-TO-USE.md`). If code diverges from a doc, fix the doc. Verify facts against source before trusting a note.
...
cd OnTime_Backend && dotnet test         # integration tests (Docker required)
```
