...
| Repo | Stack | Role |
|---|---|---|
| **OnTimeCRM** *(this repo)* | Docker Compose | Orchestrates the full stack for local/demo use |
| [OnTimeCRM_Backend](https://github.com/Gabzers/OnTimeCRM_Backend) | ASP.NET Core 8, EF Core, PostgreSQL | REST API, business logic, persistence |
| [OnTimeCRM_Frontend](https://github.com/Gabzers/OnTimeCRM_Frontend) | React 18, TypeScript, Ant Design 5 | SPA client |

## Architecture at a glance

```
┌─────────────────────┐        ┌──────────────────────┐        ┌─────────────────┐
│  OnTimeCRM_Frontend  │  HTTP  │   OnTimeCRM_Backend   │  SQL   │   PostgreSQL    │
...
- **Backend** follows Clean Architecture (`Domain ← Application ← Infrastructure ← API`), with simple CRUD in EF Core and complex aggregate logic in PostgreSQL stored functions. No EF migrations — schema is created and reconciled at startup. Tenant isolation (Company → Brand → User) is enforced through a single `AccessScope` claims helper rather than ad-hoc checks per controller. Full details: [Backend README](https://github.com/Gabzers/OnTimeCRM_Backend).
- **Frontend** is a code-split SPA built on two generic components (`ResourcePage<T>`, `CrudModal`) that most CRUD screens configure rather than reimplement, with TanStack Query owning server state and Zustand reserved for session/theme/i18n. Every UI string comes from the backend's `/api/i18n` endpoint — there's no hardcoded copy. Full details: [Frontend README](https://github.com/Gabzers/OnTimeCRM_Frontend).
...
```bash
git clone https://github.com/Gabzers/OnTimeCRM.git
cd OnTimeCRM
...
services:
  ...
  api:        # built from ./OnTimeCRM_Backend/Dockerfile
  frontend:   # built from ./OnTimeCRM_Frontend/Dockerfile, depends_on api
```
...
cd OnTimeCRM_Backend/src/OnTimeCRM.API && dotnet run     # http://localhost:5085
cd OnTimeCRM_Frontend && npm install && npm run dev       # http://localhost:5173
```
...
- [OnTimeCRM_Backend](https://github.com/Gabzers/OnTimeCRM_Backend) — REST API, Clean Architecture, integration tests
- [OnTimeCRM_Frontend](https://github.com/Gabzers/OnTimeCRM_Frontend) — React SPA, component architecture, i18n
